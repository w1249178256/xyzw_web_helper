...
// 物品ID参考表
const normalRodFromItems = computed(() => getItemCount(items.value, 1011)); // 普通鱼竿
const goldRodFromItems = computed(() => getItemCount(items.value, 1012)); // 黄金鱼竿
const pearlFromItems = computed(() => getItemCount(items.value, 1013)); // 珍珠
const recruitFromItems = computed(() => getItemCount(items.value, 1001)); // 招募令
const ironFromItems = computed(() => getItemCount(items.value, 1006)); // 精铁
const jadeFromItems = computed(() => getItemCount(items.value, 1023)); // 彩玉
const advanceStoneFromItems = computed(() => getItemCount(items.value, 1003)); // 进阶石
const DanFromItems = computed(() => getItemCount(items.value, 1017)); // 复活丹
const blueJadeFromItems = computed(() => getItemCount(items.value, 10002)); // 四圣蓝玉
const redJadeFromItems = computed(() => getItemCount(items.value, 10003)); // 四圣红玉
const fourSaintFragmentFromItems = computed(() => getItemCount(items.value, 10101)); // 四圣宝珠碎片
...
const indigoFromItems = computed(() => getItemCount(items.value, 1019)); // 盐锭
...
const whiteJadeFromItems = computed(() => getItemCount(items.value, 1022)); // 白玉
const shellFromItems = computed(() => getItemCount(items.value, 1033)); // 贝壳
const goldIndigoFromItems = computed(() => getItemCount(items.value, 1035)); // 金盐锭
...
const resList = computed(() => {
  const allResources = [
    ...
    { label: '盐锭', value: display(indigoFromItems.value as any), raw: getRawValue(indigoFromItems.value as any) },
    ...
    { label: '金盐锭', value: display(goldIndigoFromItems.value as any), raw: getRawValue(goldIndigoFromItems.value as any) },
