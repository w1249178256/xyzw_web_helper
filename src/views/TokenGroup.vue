...
const handleAddTokensToGroup = () => {
  if (!selectedAddGroupId.value || !addTokenRange.value) {
    message.warning('请选择分组并输入Token范围')
    return
  }
  
  const range = addTokenRange.value.trim()
  ...
  const tokensToAdd = Array.from(indices).map(i => allTokens[i - 1]).filter(Boolean)
  
  if (tokensToAdd.length === 0) {
    message.warning('没有找到有效的Token')
    return
  }
  
  const group = tokenGroups.value.find(g => g.id === selectedAddGroupId.value)
  if (!group) {
    message.error('分组不存在')
    return
  }
  ...
  message.success(`成功添加 ${addedCount} 个 Token 到分组 "${group.name}"`)
  addTokenRange.value = ''
}
...
const handleClearGroupTokens = () => {
  ...
  const tokenCount = group.tokenIds.length
  group.tokenIds = []
  
  message.success(`已清空分组 "${group.name}" 中的 ${tokenCount} 个 Token`)
  selectedClearGroupId.value = null
}
...
const batchSettings = reactive({
  ...
  // 延迟配置（毫秒）
  commandDelay: 600, // 命令间延迟
  taskDelay: 600, // 任务间延迟
  actionDelay: 600, // 一般操作延迟（开箱、钓鱼、招募等）
  battleDelay: 600, // 战斗延迟（宝库、竞技场等）
